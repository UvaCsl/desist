...
```
The package requires Python version `3.8`. If Python is not available, or only
an older version is present, it can be installed as:
```
# for macOS
...
a local, virtual environment. Depending on your system's settings, you might
need to replace `python3` with `python3.8` to indicate the right version. To
(optionally) install the virtual environment, first run
```
...
thorough verification, you can evaluate the tests:
```
pytest --cov=workflow
```
...
```
Building containers with Docker requires root permissions and therefore `sudo`
is prefixed for the `docker build` and `docker run` commands, potentially
requesting a password. If the current user is part of a user group with access
to Docker, you can append the `--root` flag to suppress any `sudo` prefixes.
...
- `-j2`: allow two simultaneous jobs on the remote machine
